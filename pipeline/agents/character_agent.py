"""CharacterAgent — audits character arc and relationship-state consistency."""
...
class CharacterAgentOutput(BaseModel):
    scene_id: str = ""
    arc_consistency_score: float = Field(default=1.0, ge=0.0, le=1.0)
...
class CharacterAgent(BaseAgent):
    ...
    def __init__(self, ctx: AgentContext, model_router: ModelRouter) -> None:
        ...
        load_memory(ctx, "CharacterAgent")

    def _execute(self, job_context: JobContext) -> JobContext:
        arc_events = self.ctx.ledger_manager.character_arc._all_payloads()
        output: CharacterAgentOutput = self._router.call(
            messages=[
                {
                    "role": "system",
                    "content": "You are CharacterAgent. Audit arc and relationship continuity.",
                },
                {
                    "role": "user",
                    "content": (
                        f"Scene: {job_context.scene_id}\nRecent arc events: {arc_events[-5:]}\n\n"
        ...
        save_memory(self.ctx, "CharacterAgent", job_context.scene_id, {"passed": output.passed})
        logger.info("CharacterAgent: scene=%s passed=%s", job_context.scene_id, output.passed)
        return job_context.with_output("character_agent", output.model_dump())
